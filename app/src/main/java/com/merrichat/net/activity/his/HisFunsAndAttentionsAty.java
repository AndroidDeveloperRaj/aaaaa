...
/**
 * Created by Allen Cheng on 2018/4/11.
 * <p>
 * 他的粉丝
 */
public class HisFunsAndAttentionsAty extends MerriActionBarActivity implements OnRefreshListener, OnLoadmoreListener, BaseQuickAdapter.OnItemChildClickListener, BaseQuickAdapter.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        int fromHisFunsOrAttentions = getIntent().getIntExtra("fromHisFunsOrAttentions", -1);
        if (fromHisFunsOrAttentions == 0) {
            setTitle("他的关注");
            status = 0;
        } else if (fromHisFunsOrAttentions == 1) {
            setTitle("他的粉丝");
            status = 1;
        }
        setLeftBack();
    ...
    private void queryAttentionRelation() {
        OkGo.<String>get(Urls.queryAttentionOther)//
                .tag(this)//
                .params("memberId", UserModel.getUserModel().getMemberId())
                .params("otherMemberId", hisMemberId)
...
//                        refreshLayout.finishRefresh();
    ...
    public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
        ...
        long toMemberId = 0;
        if (status == 0) {//关注
            toMemberId = dataBean.getToMemberId();
        } else if (status == 1) {//粉丝
            toMemberId = dataBean.getFromMemberId();
        }
