...
public class OverOrderFragment extends BaseFragment implements OnRefreshListener, OnLoadmoreListener {
    Unbinder unbinder;
     ...
     */
    private final int orderStatus = 4;
    ...
    private void initView() {
        ...
        adapter.setOnItemClickListener(new ToBeDeliveredAdapter.OnItemClickListener() {
            ...
            public void CheckTransMapOClick(int position) {
                if (pendingRegimentList.get(position).getSendType() == 1) {
                    startActivity(new Intent(cnt, ChaKanWuLiuActivity.class).putExtra("orderId",pendingRegimentList.get(position).getOrderId()));
                } else if (pendingRegimentList.get(position).getSendType() == 2) {
                    GetToast.useString(cnt, "此订单自取");
                } else if (pendingRegimentList.get(position).getSendType() == 3) {
