...
public class SelectFriendsAdapter extends BaseListAdapter<GoodFriendModel> implements SectionIndexer {
    List<GoodFriendModel> list;
    private Context mContext;

    public SelectFriendsAdapter(Context context, List<GoodFriendModel> data) {
        super(context, data);
        this.list = data;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        GroupAdminstorAdapter.ViewHolder viewHolder = null;
        if (convertView == null) {
            viewHolder = new GroupAdminstorAdapter.ViewHolder();
            ...
            viewHolder = (GroupAdminstorAdapter.ViewHolder) convertView.getTag();
        }
        ...
        if (position == getPositionForSection(section)) {
            ...
            viewHolder.tvCatalog.setText(list.get(position).firstLetter);
        } else {
        ...
        if (list.get(position).flag) {
            viewHolder.imageViewCheck.setImageResource(R.mipmap.shaixuan_weixuanzhong2x);
        } else {
        ...
        viewHolder.tvName.setText(this.list.get(position).goodFriendsName);
        viewHolder.simpleHeader.setImageURI(this.list.get(position).goodFriendsUrl);

        viewHolder.relGroup.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                onGroupItemClickLinster.onItemClick(position,list.get(position));
            }
        });
    ...
    public int getSectionForPosition(int position) {
        return list.get(position).firstLetter.charAt(0);
    }
    ...
    public interface onGroupItemClickLinster {
        void onItemClick(int position,GoodFriendModel goodFriendModel);
    }
}
