...
public class AudioRecorderButton extends Button implements AudioManage.AudioStateListenter {
    ...
    public AudioRecorderButton(Context context, AttributeSet attrs) {
        ...
        File voiceFile = FolderManager.getVoiceFolder();                          // 此处需要判断是否有存储卡(外存)
        if (voiceFile == null) {

            GetToast.useString(context, "存储路径错误");
        }
        mAudioManage = AudioManage.getInstance(voiceFile);
    ...
    public interface AudioFinishRecorderListenter {
         ...
         * 录音完成后的回调方法
         *
         * @param @param seconds 声音文件的长度
         * @param @param FilePath 保存的路径
         * @return void    返回类型
         */
     ...
     * 时间超时，完成
     */
    private static final int MSG_OUT_TIME_FINISH = 0x113;
        ...
        public void run() {
            ...
            while (isRecordering) {
                try {
                    Thread.sleep(100);
                    mTime += 0.1f;
                    LogUtil.d("@@@", "mTime===" + mTime);
                    mHandler.sendEmptyMessage(MSG_VOICE_CHANGE);
                    if (mTime > OUT_TIME) {
                        mHandler.sendEmptyMessage(MSG_OUT_TIME_FINISH);
                        isRecordering = false;
                    }
