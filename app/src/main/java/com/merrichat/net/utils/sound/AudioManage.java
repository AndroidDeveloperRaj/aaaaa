...
public class AudioManage {
    ...
    private AudioManage(File dirFile) {
    ...
    public static AudioManage getInstance(File dirFile) {
        if (mInstance == null) {
            synchronized (AudioManage.class) { // 同步
                if (mInstance == null) {
                    mInstance = new AudioManage(dirFile);
                }
            }
        }
    ...
    public void prepareAudio() {
        ...
        try {
            ...
            File file = new File(dirFile, fileName);  // 路径+文件名字
