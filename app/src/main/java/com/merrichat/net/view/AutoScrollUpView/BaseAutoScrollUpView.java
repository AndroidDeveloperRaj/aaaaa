 ...
 * @param <T>
 */
public abstract class BaseAutoScrollUpView<T> extends ListView implements AutoScrollData<T> {
    /**
     * 数据源
     */
    ...
    public BaseAutoScrollUpView(Context context, AttributeSet attrs,int defStyle) {
    ...
    public BaseAutoScrollUpView(Context context, AttributeSet attrs) {
    ...
    private class AutoScrollAdapter extends BaseAdapter {
        ...
        public View getView(final int position, View convertView,
            ...
            if (null == convertView) {
                ...
                convertView = LayoutInflater.from(mContext).inflate(R.layout.item_scrollup_view, null);
                viewHolder.mInfoView = (TextView) convertView.findViewById(R.id.tv_tip);
                viewHolder.mIv = (ImageView) convertView.findViewById(R.id.iv_tip_img);
                convertView.setTag(viewHolder);
            } else {
                viewHolder = (ViewHolder) convertView.getTag();
            }
            T data = mDataList.get(position % mMax);
//            viewHolder.mTitleView.setLayoutParams(new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.WRAP_CONTENT,dip2px(getAdertisementHeight())));
            viewHolder.mInfoView.setTextSize(mSize);
            Glide.with(mContext).load(getIvUrl(data)).into(viewHolder.mIv);
