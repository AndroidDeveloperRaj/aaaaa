...
public class SwipeBackLayout extends FrameLayout {
    ...
    private boolean mEnable = true;   // 是否支持滑动返回
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                ...
                if (moveX - downX >= 0 && isSilding) {
                    if (mOnFinishListener == null) {
                        ...
                        mOnFinishListener.doIfShouldScroll(mContentView, deltaX);
                    }
                }
                break;
            case MotionEvent.ACTION_UP:
                ...
                if (mContentView.getScrollX() <= -viewWidth / 3) {
                    ...
                    if (mOnFinishListener != null) {
                        mOnFinishListener.doIfShouldScrollOver();
                    } else {
                        scrollRight();
                    }
                } else {
    ...
    public void scrollRight() {
        ...
        mScroller.startScroll(mContentView.getScrollX() - 128, 0, -delta + 1, 0,
                Math.abs(delta));
        postInvalidate();
    }
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if (mScroller.isFinished() && isFinish) {
                if (mOnFinishListener != null) {
                    mOnFinishListener.doIfShouldFinish();
                } else {
                    mActivity.finish();
                }
            }
    ...
    public void setmOnFinishListener(OnFinishListener listener) {
        mOnFinishListener = listener;
    }

    public interface OnFinishListener {
        void doIfShouldScroll(View contentView, int deltaX);

        void doIfShouldScrollOver();

        void doIfShouldFinish();
    }
}
