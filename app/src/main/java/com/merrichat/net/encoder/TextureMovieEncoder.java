...
public class TextureMovieEncoder {
    ...
    private static class VideoEncoderHandler extends Handler {
        ...
        public void handleMessage(Message inputMessage) {
            ...
            if (encoder == null) {
                Logger.w(TAG, "VideoEncoderHandler.handleMessage: encoder is null");
                return;
            }
    ...
    private void handleFrameAvailable(float[] transform, long timestampNanos) {
        if (texture != 0) {
            if (VERBOSE) Logger.e(TAG, "handleFrameAvailable " + timestampNanos);
            if (VERBOSE) Logger.d(TAG, "handleFrameAvailable tr=" + transform);
            try {
            ...
            //Log.d("LogTest", "getPTSUs() * 1000＝＝＝＝＝》" + getPTSUs() * 1000L + "--->" + i ++);
    ...
    private void handleStopRecording() {
        ...
        releaseEncoder();
        Logger.e(TAG, "handleStopRecording before stop success");
        ...
        public void run() {
            ...
            try {
                ...
                if (audioRecord != null) {
                    try {
                        if (VERBOSE) {
                            Logger.v(TAG, "AudioThread:start audio recording");
                        }
                    ...
                    Logger.e(TAG, "failed to initialize AudioRecord");
                }
            } catch (final Exception e) {
                Logger.e(TAG, "AudioThread#run:" + e.toString());
            }
            if (VERBOSE) {
                Logger.v(TAG, "AudioThread:finished");
            }
     ...
     *
     * 快速录制就是根据第一帧的时间戳，得出当前帧对应的当前时间与第一帧时间差的一半，
     * 加上第一帧的时间戳，即为正确的时间戳。
     *
     * 慢速录制的时候时间戳就是第一帧时间戳，加上已经渲染的帧数乘上帧间隔即可。
     *
     * @return
     */
    protected long getPTSUs() {
        long result, thisNanoTime = System.nanoTime();

        if (firstTimeStampBase == 0) {
            result = thisNanoTime;
        } else {
            if (firstNanoTime == 0) {
                firstNanoTime = thisNanoTime;
            }
        ...
        //Logger.e("getPTSUs+" + result);


        return prevOutputPTSUs = result;

        //        long time = System.nanoTime();
//        return firstTimeStampBase + (result - firstTimeStampBase) / 4;
    }
