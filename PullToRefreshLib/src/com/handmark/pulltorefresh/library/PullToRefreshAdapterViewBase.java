...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView>
	...
	private IndicatorLayout mCalendarIvTop;
			...
			if (firstVisibleItem > lastVisibleItemPosition
				...
				if (null != mIndicatorIvTop) {
					if (mIndicatorIvTop.isVisible()) {
						...
						if (null != mCalendarIvTop) {
							if (!mCalendarIvTop.isVisible()) {
								mCalendarIvTop.show();
							}
						}
					}
	...
	public final void onScrollStateChanged(final AbsListView view,
		...
		if (state == OnScrollListener.SCROLL_STATE_IDLE
				&& null != mOnFirstItemVisibleListener && mFirstItemVisible) {
	...
	public final void setOnFirstItemVisibleListener(
			OnFirstItemVisibleListener listener) {
	...
	private void addIndicatorViews() {
		...
		if (mode.showHeaderLoadingLayout() && null == mIndicatorIvTop) {
			...
			mIndicatorIvTop = new IndicatorLayout(getContext(),
					Mode.PULL_FROM_START, IndicatorLayout.INDICATOR_TOP);
			...
			mCalendarIvTop = new IndicatorLayout(getContext(),
					Mode.PULL_FROM_START, IndicatorLayout.INDICATOR_CAL);
			refreshableViewWrapper.addView(mCalendarIvTop, params);
			mIndicatorIvTop.setVisibility(View.VISIBLE);
			mCalendarIvTop.setVisibility(View.GONE);

		} else if (!mode.showHeaderLoadingLayout() && null != mIndicatorIvTop) {
			// If we can't pull down, but have a View then remove it
			refreshableViewWrapper.removeView(mIndicatorIvTop);
			refreshableViewWrapper.removeView(mCalendarIvTop);
